% Baseline Population of Models — FEMALE ventricular model (no drug).
...
for i = 1:nsmp
    params(i).ICaL_Multiplier  = LHSR(i,1) * F_ICaL;
    params(i).INa_Multiplier   = LHSR(i,2);
    params(i).INaCa_Multiplier = LHSR(i,3) * F_INaCa;
    params(i).INaK_Multiplier  = LHSR(i,4);
    params(i).IKr_Multiplier   = LHSR(i,5) * F_IKr;
    params(i).IKs_Multiplier   = LHSR(i,6) * F_IKs;
    params(i).Ito_Multiplier   = LHSR(i,7) * F_Ito;
    params(i).IK1_Multiplier   = F_IK1;
end
...
writetable(T, fullfile(outdir, 'baseline_female.csv'));
fprintf('Saved POM_results/baseline_female.csv\n');
