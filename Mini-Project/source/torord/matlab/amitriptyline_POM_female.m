% Amitriptyline Population of Models — FEMALE ventricular model.
...
%   INaL ×1.00 (unchanged baseline; drug block still applies)
...
for i = 1:nsmp
    ...
    params(i).Ito_Multiplier   = LHSR(i,7) * F_Ito   * coef(C, IC50_Ito,  h_Ito);
    params(i).INaL_Multiplier  = coef(C, IC50_INaL, h_INaL);  % INaL female scaling = 1.00
    params(i).IK1_Multiplier   = F_IK1;  % female scaling only; not drug-blocked
end
...
title('Action Potential — Female population of models for a maximum therapeutic dose of Amitryptiline');
xlabel('Time (ms)');
...
title('Calcium Transient — Amitriptyline max dose, Female POM');
xlabel('Time (ms)');
ylabel('Ca_i (mM)');
...
writetable(T, fullfile(outdir, 'amitriptyline_female.csv'));
fprintf('Saved POM_results/amitriptyline_female.csv\n');
